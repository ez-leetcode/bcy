...
public interface CosPlayMapper extends BaseMapper<CosPlay> {
    ...
    //要注意仅自己可见不能展示
    @Select("SELECT a.id,a.number,a.description FROM cos_play as a AND fans as b WHERE b.from_id = #{id} AND a.id = b.to_id AND a.permission != 3 ORDER BY a.create_time DESC")
